...
<body>
    ...
    <main class="col-md-12 ms-sm-auto col-lg-10 px-5">
        <div class="row g-5">
            <div class="col-md-7 col-lg-8">
                ...
                <form action="php/att_update.php" class="" novalidate method="post" enctype="multipart/form-data">
                <div class="row g-3">
                    <div class="col-12">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="att_Address" name="att_Address" placeholder="600嘉義市東區中正路361號(地址請輸入完整!!)" required>
                            <div class="invalid-feedback">
                            請重新輸入.
                            </div>
                    </div>
                    ...
                    <div class="col-sm-12" style="padding-right:24px;">
                        <input class="form-check-input" type="checkbox" value="on" name="alltimecheckbox" id="defaultCheck1">
                    ...
                    <input type="time" class="form-control" id="endtime"  name='att_Endtime' placeholder="24:00:00" value="" required oninput="validateTime(this)">
                    <div class="invalid-feedback">
                    請重新輸入.
                    </div>
                    ...
                    <div class="row">
                        <!-- 禮拜一 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxMonday" value="1" checked>
                            <label class="form-check-label" for="inlineCheckboxMonday">週一</label>
                        </div>

                        <!-- 禮拜二 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxTuesday" value="2" checked>
                            <label class="form-check-label" for="inlineCheckboxTuesday">週二</label>
                        </div>

                        <!-- 禮拜三 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxWednesday" value="3" checked>
                            <label class="form-check-label" for="inlineCheckboxWednesday">週三</label>
                        </div>

                        <!-- 禮拜四 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxThursday" value="4" checked>
                            <label class="form-check-label" for="inlineCheckboxThursday">週四</label>
                        </div>

                        <!-- 禮拜五 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxFriday" value="5" checked>
                            <label class="form-check-label" for="inlineCheckboxFriday">週五</label>
                        </div>

                        <!-- 禮拜六 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxSaturday" value="6">
                            <label class="form-check-label" for="inlineCheckboxSaturday">週六</label>
                        </div>

                        <!-- 禮拜日 -->
                        <div class="col">
                            <input class="form-check-input" type="checkbox" name="days[]" id="inlineCheckboxSunday" value="7">
                            <label class="form-check-label" for="inlineCheckboxSunday">週日</label>
                        </div>
                    </div>
                </div>
                    ...
                    <label for="endtime" class="form-label">地點分類</label><br>
                    <input class="form-control" list="locationTags" id="tagNameInput" oninput="setLocationTagId()" autocomplete="off" >
                    <datalist id="locationTags">                       <?php
                        ...
                        if (!empty($attractionsData)) {
                            foreach ($attractionsData as $attraction) {
                                if ($attraction['tag_state'] == 1) {
                                    echo "<option data-id='".$attraction['tag_id']."' value='".$attraction['tag_name']."'></option>";
                                }
                            }
                        } else {
                            echo "<option disabled>沒有資料。</option>";
                        }
                        ?>
                    </datalist>

                    <!-- 隱藏的輸入框，用於儲存 tag_id -->
                    <input type="hidden" id="locationTagIdHidden" name="att_tag_1">


                </div>

                <div class="col-sm-3 py-2">
                    <label for="endtime" class="form-label">飲食分類</label><br>
                    <input class="form-control" list="foodTags" oninput="setFoodTagId()" name="att_tag_2" autocomplete="off">
                    <datalist id="foodTags">
                        <?php
                        if (!empty($attractionsData)) {
                            foreach ($attractionsData as $attraction) {
                                if ($attraction['tag_state'] == 2) {
                                    echo "<option data-id='".$attraction['tag_id']."' value='".$attraction['tag_name']."'></option>";
                                }
                            }
                        }
                ...
                <!-- 隱藏的輸入框，用於儲存 tag_id -->
                <input type="hidden" id="foodTagIdHidden" name="att_tag_2">
                </div>

                <div class="col-sm-3 py-2">
                    <label for="endtime" class="form-label">客群分類</label><br>
                    <input class="form-control" list="audienceTags" oninput="setAudienceTagId()" name="att_tag_3" onchange="syncDatalist() autocomplete="off">
                    <datalist id="audienceTags">
                    <?php
                        if (!empty($attractionsData)) {
                            foreach ($attractionsData as $attraction) {
                                if ($attraction['tag_state'] == 3) {
                                    echo "<option data-id='".$attraction['tag_id']."' value='".$attraction['tag_name']."'></option>";
                                }
                            }
                        }
                ...
                <!-- 隱藏的輸入框，用於儲存 tag_id -->
                <input type="hidden" id="audienceTagIdHidden" name="att_tag_3">
                </div>
                    ...
                    <div class="col-4">
                        <label for="att_freetag" class="form-label">TAG(請用#區分)</label><br>
                        <input name="att_freetag" class="form-control" type="text" placeholder="#超好玩#一定要來#放空耍廢">
                            <div class="invalid-feedback">
                            請重新輸入.
                            </div>
                    </div>
    ...
    <script> 
        ...
        function setLocationTagId() {
                const input = document.getElementById('tagNameInput');
                const selectedOption = Array.from(document.querySelectorAll('#locationTags option')).find(option => option.value === input.value);
                const tagIdHidden = document.getElementById('locationTagIdHidden');
                var validOption = false;
                
                if (selectedOption) {
                    tagIdHidden.value = selectedOption.getAttribute('data-id');
                } else {
                    tagIdHidden.value = '';
                }
                
            }
            function setFoodTagId() {
                const input = document.querySelector('[name="att_tag_2"]');
                const selectedOption = Array.from(document.querySelectorAll('#foodTags option')).find(option => option.value === input.value);
                const tagIdHidden = document.getElementById('foodTagIdHidden');
            ...
            function setAudienceTagId() {
                const input = document.querySelector('[name="att_tag_3"]');
                const selectedOption = Array.from(document.querySelectorAll('#audienceTags option')).find(option => option.value === input.value);
                const tagIdHidden = document.getElementById('audienceTagIdHidden');
            ...
            function setfreeTagId() {
                const input = document.querySelector('[name="att_tag_4_name"]');
                const selectedOption = Array.from(document.querySelectorAll('#freeTags option')).find(option => option.value === input.value);
                const tagIdHidden = document.getElementById('freeTagIdHidden');
            ...
            function syncDatalist() {
                var selectElement = document.getElementById('audienceSelect');
