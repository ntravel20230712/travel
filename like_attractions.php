...
<body>
  ...
  <main class="col-md-12 ms-sm-auto px-md-4">
    <div
      ...
      // 查询用户收藏的景点信息
      $query = "SELECT COUNT(*) AS totalFavorites FROM favorites WHERE mem_id = ?";
      ...
      <!-- 将已收藏的景点数量显示在指定位置 -->
      <strong class="d-block py-2">已查到<?php echo $totalFavorites; ?>個景點 </strong>
      ...
      <!-- 排列bar結束 -->
      <script><?php require_once('php/getfavorites.php'); ?>
        var favoriteCounts = <?php echo json_encode($favoriteCounts); ?>;</script>
<?php
      // 创建数据库连接
      require_once('php/db_connect_wamp.php');
...
// 检索用户的收藏景点信息
$query = "SELECT f.id, f.mem_id, f.att_id, f.created_at, a.att_id, a.att_name, a.att_City, a.att_type FROM favorites f INNER JOIN attractions a ON f.att_id = a.att_id WHERE f.mem_id = ?";
$stmt = $con->prepare($query);
$stmt->bind_param("i", $memberId); // 将会员 ID 绑定到查询参数
$stmt->execute();
$result = $stmt->get_result();
...
if ($result->num_rows > 0) {
    ...
    echo '                        <tr>';
    echo '                            <th scope="col">景點名稱</th>';
    echo '                            <th scope="col">城市</th>';
    echo '                            <th scope="col">旅遊類型</th>';
    echo '                            <th scope="col">收藏日期</th>';
    echo '                            <th scope="col">景點已被收藏數</th>';
    ...
    while ($row = $result->fetch_assoc()) {
      // 将favoriteCounts[$row['att_id']]保存在临时变量$favoriteCount中
      $favoriteCount = isset($favoriteCounts[$row['att_id']]) ? $favoriteCounts[$row['att_id']] : 0;
      echo '                        <tr>';
      echo '                            <td>' . $row['att_name'] . '</td>';
      echo '                            <td>' . $row['att_City'] . '</td>';
      echo '                            <td>' . $row['att_type'] . '</td>';
      echo '                            <td>' . $row['created_at'] . '</td>';
      echo '                            <td>' . $favoriteCount . '</td>'; // 使用$favoriteCount变量，而不是$favoriteCounts
      echo '<td><a href="big_attraction.php#' . htmlspecialchars($row['att_name'], ENT_QUOTES) . '" onclick="increaseViews(\'' . htmlspecialchars($row['att_name'], ENT_QUOTES) . '\')">點擊查看</a></td>';
...
function sortTable(sortValue) {
  ...
  switch (sortValue) {
    case '0': // 最多收藏
      rows.sort((a, b) => {
        const valueA = parseInt(a.cells[4].textContent);
        const valueB = parseInt(b.cells[4].textContent);
