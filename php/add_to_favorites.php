...
if ($result->num_rows > 0) {
    ...
    $stmt = $con->prepare("SELECT * FROM favorites WHERE mem_id = ? AND att_id = ?");
    ...
    if ($result->num_rows > 0) {
        // 已收藏，执行取消收藏操作
        $stmt = $con->prepare("DELETE FROM favorites WHERE mem_id = ? AND att_id = ?");
        $stmt->bind_param("ii", $memID, $attractionID);
        if ($stmt->execute()) {
            // 取消收藏成功
            echo "unfavorited";
        ...
        // 未收藏，执行添加收藏操作
        $stmt = $con->prepare("INSERT INTO favorites (mem_id, att_id) VALUES (?, ?)");
        $stmt->bind_param("ii", $memID, $attractionID);
        if ($stmt->execute()) {
            // 收藏成功
            echo "favorited";
